...
export function SearchInput() {
   ...
   return (<form className="flex flex-wrap gap-4" onSubmit={(e) => e.preventDefault()}>
      <div className="relative flex-grow">
         <input
            className="box-border w-full px-6 py-3 pr-12 border-2 border-indigo-800 rounded-md shadow-inner bg-slate-900 focus:border-indigo-400 focus:border-2 focus:shadow-outline focus:ring-indigo-50 focus:ring-4 focus:outline-none text-indigo-50 sm:text-lg placeholder:text-indigo-50/30"
            autoFocus
            value={query}
            onKeyUp={(e) => {
               if (e.key === "Enter") {
